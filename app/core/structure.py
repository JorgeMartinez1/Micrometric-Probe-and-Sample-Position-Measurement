...
def compute_H_3D_5_points(p3d_p, p3d):
    """ Calcula la transformación proyectiva H de dos juegos de correspondencias 3D.
    """
    n = len(p3d_p)
    if n < 5 or len(p3d) != n:
        raise ValueError('Number of points do not match.')
    ...
    for i in range(5):
        A[3*i, 0:16] = np.array([p3d[i][0], p3d[i][1], p3d[i][2], 1, 0, 0, 0, 0, 0, 0, 0, 0, -(p3d[i][0] * p3d_p[i][0]), -(p3d[i][1] * p3d_p[i][0]), -(p3d[i][2] * p3d_p[i][0])])
        A[3*i + 1, 0:16] = np.array([0, 0, 0, 0, p3d[i][0], p3d[i][1], p3d[i][2], 1, 0, 0, 0, 0, -(p3d[i][0] * p3d_p[i][1]), -(p3d[i][1] * p3d_p[i][1]), -(p3d[i][2] * p3d_p[i][1])])
        A[3*i + 2, 0:16] = np.array([0, 0, 0, 0, 0, 0, 0, 0, p3d[i][0], p3d[i][1], p3d[i][2], 1, -(p3d[i][0] * p3d_p[i][2]), -(p3d[i][1] * p3d_p[i][2]), -(p3d[i][2] * p3d_p[i][2])])

    A_inv = np.linalg.inv(A)

    H_3D = np.dot(A_inv, np.array(p3d_p).flatten())

    H_3D = np.insert(H_3D, 15, 1)

    return H_3D.reshape((4,4))

def compute_H_3D_overdeterminated(p3d_p, p3d):
    """ Calcula la transformación proyectiva H de dos juegos de correspondencias 3D.
    """
    n = len(p3d_p)
    if n <= 5 or len(p3d) != n:
...
data_patron = open(f"{os.getcwd()}{os.sep}outputs{os.sep}tdparams{os.sep}puntos3DPatron.txt").read()
lines_patron = data_patron.split('\n')

data_estimados = open(f'{os.getcwd()}{os.sep}outputs{os.sep}tdparams{os.sep}puntos3DEstimados.txt').read()
lines_estimados = data_estimados.split('\n')

p3d = []
i = 0
for line in lines_patron:
    if i <= 63:
        try:
            xp, yp, zp = line.split(',')  # Delimiter is comma

            p3d.append((float(xp), float(yp), float(zp)))
            i = i + 1
...
p3d_p = []
i = 0
for line in lines_estimados:
    if i <= 63:
        try:
            xe, ye, ze = line.split(',')  # Delimiter is comma
            p3d_p.append((float(xe), float(ye), float(ze)))
            i = i+1

        except Exception as e:
            print("Error leyendo mediciones", e)

h_3d = compute_H_3D_overdeterminated(p3d_p, p3d)
# print(h_3d)
